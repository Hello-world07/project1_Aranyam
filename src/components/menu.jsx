...
const dishes = [
  ...
  { id: 12, category: 'biryani', veg: false, name: 'Mutton Dum Biryani', desc: 'Tender mutton pieces cooked in dum style with aromatic spices', price: 460, image: '/Mutton Dum Biryani.jpg' },
...
export default function Menu({ onBookTable }) {
  ...
  // All 3 filters work together simultaneously
    ...
    <>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="max-w-xl mx-auto mb-6">
            <div className="relative group">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gold-500/70
                group-focus-within:text-gold-400 transition-colors duration-300" />
              <input
                ref={searchRef}
                type="text"
                value={searchQuery}
                ...
                className="w-full pl-11 pr-11 py-3.5
                  bg-jungle-900/70 border border-jungle-600/50
                  rounded-full font-cormorant text-lg text-jungle-100
                  placeholder:text-jungle-400/60
                  focus:outline-none focus:border-gold-500/60
                  focus:bg-jungle-900/90
                  focus:shadow-[0_0_20px_rgba(212,160,23,0.15)]
              ...
              {searchQuery && (
                <button
                  ...
                  className="absolute right-4 top-1/2 -translate-y-1/2
                    w-5 h-5 rounded-full bg-jungle-600/60 hover:bg-gold-500/80
            ...
            {vegFilters.map(({ id, label, dot }) => (
              <button
                ...
                className={`flex items-center gap-2 px-5 py-2 rounded-full
                  font-cinzel text-xs tracking-[0.12em] uppercase
                  transition-all duration-300
                  ${vegFilter === id
              ...
              >
                {dot && (
                  <span className={`w-2.5 h-2.5 rounded-full flex-shrink-0
                    ${dot === 'green' ? 'bg-green-400' : 'bg-red-500'}`}
                  />
                )}
                {label}
              </button>
          ...
          {/* ── Category Tabs ── */}
          <div
            className="flex items-center gap-2 sm:gap-3 overflow-x-auto pb-3 mb-6"
          ...
          >
            {categories.map(({ id, label }) => (
              <button
                ...
                className={`relative flex-shrink-0 px-4 sm:px-5 py-2 sm:py-2.5 rounded-full
                  font-cinzel text-xs sm:text-sm tracking-[0.12em] uppercase
          ...
          <div className="flex items-center justify-between mb-8 px-1">
            ...
            {hasActiveFilters && (
              <button
                onClick={resetAll}
                className="flex items-center gap-1.5 font-cinzel text-xs tracking-[0.1em] uppercase
                  text-jungle-400 hover:text-gold-400 transition-colors duration-200"
              >
                <X className="w-3 h-3" />
                Reset
              </button>
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-20">
              <div className="w-16 h-16 mx-auto mb-5 rounded-full bg-jungle-900/60
                border border-jungle-700/50 flex items-center justify-center">
                <Search className="w-7 h-7 text-jungle-500" />
              </div>
              <p className="font-cinzel text-gold-400/60 text-lg tracking-[0.15em] mb-2">
                No Dishes Found
              </p>
              <p className="font-cormorant text-jungle-400 text-lg">
                Try a different search or filter
              </p>
              <button
                onClick={resetAll}
                className="mt-6 px-6 py-2.5 rounded-full font-cinzel text-xs tracking-[0.15em] uppercase
                  text-jungle-950 font-bold
                  bg-gradient-to-r from-gold-500 via-gold-400 to-gold-600
                  shadow-[0_0_16px_rgba(212,160,23,0.4)]
                  hover:shadow-[0_0_28px_rgba(212,160,23,0.7)]
              ...
              {filtered.map((dish) => (
                <div
                  ...
                  className="relative group rounded-2xl overflow-hidden
                    border border-jungle-700/50 bg-jungle-900/70
                    hover:border-gold-600/50
                    transition-all duration-500
                    shadow-[0_4px_20px_rgba(0,0,0,0.5)]
                    hover:shadow-[0_8px_32px_rgba(0,0,0,0.7),0_0_20px_rgba(212,160,23,0.12)]
                    hover:-translate-y-1"
                >
                  {/* Dish image */}
                  <div className="relative overflow-hidden h-48 sm:h-52 bg-jungle-900">
                    ...
                    <div className="absolute top-3 left-3">
                      <div className={`w-5 h-5 rounded-sm border-2 flex items-center justify-center bg-black/80
                        ${dish.veg ? 'border-green-400' : 'border-red-500'}`}>
                    ...
                    <div className="absolute top-3 right-3">
                      <span className="font-cinzel text-xs font-bold text-jungle-950
                  ...
                  <div className="p-4 sm:p-5">
                    <h3 className="font-cinzel text-sm sm:text-base tracking-[0.1em] text-gold-400 font-semibold mb-2
                      group-hover:text-gold-300 transition-colors duration-300">
                      {dish.name}
                    </h3>
                    <p className="font-cormorant text-jungle-100 text-base sm:text-lg leading-relaxed line-clamp-2">
                      {dish.desc}
                    </p>
                  </div>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 z-50
          ...
          text-jungle-950
          flex items-center justify-center
          shadow-[0_0_20px_rgba(212,160,23,0.5)]
          hover:shadow-[0_0_32px_rgba(212,160,23,0.8)]
